...
public class StingyTrader : Trader
{
    ...
    protected override void MakeCounter(string input, int inputCount, string output, int outputCount)
    {
        ...
        if (stockedInput < inputCount && stockedInput > 0)
        {
            ...
            // await Brain.GetTradeDecisionAsync(output, stockedInput, input, int (stockedInput * ProfitMargin), traderType);
        }
        else if (foodStock > 0)
        {
            // Offers food for same input
            // await Brain.GetTradeDecisionAsync("food", GetStock("food"), input, int (GetStock("food") * ProfitMargin), traderType);
        }
        else if (waterStock > 0)
        {
            // Offers water for same input
            // await Brain.GetTradeDecisionAsync("water", GetStock("water"), input, int (GetStock("water") * ProfitMargin), traderType);
        }
        else if (goldStock > 0)
        {
            // Offers gold for same input
            // await Brain.GetTradeDecisionAsync("gold", GetStock("gold"), input, int (GetStock("gold") * ProfitMargin), traderType);
        }
    }
}
